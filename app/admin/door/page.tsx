...
type Attendee = Pick<Registration, 'id' | 'first_name' | 'last_name' | 'email' | 'country' | 'status' | 'checkin_at' | 'rsvp_token'>
...
export default function DoorPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('registrations')
      .select('id, first_name, last_name, email, country, status, checkin_at, rsvp_token')
      .eq('event_id', eventId)
  ...
  async function startScanner() {
    ...
    await scanner.start(
      ...
      async (decodedText: string) => {
        await stopScanner()
        // QR contains rsvp_token
        const found = attendees.find((a) => a.rsvp_token === decodedText)
        if (found) {
          setScanResult({ ok: true, name: `${found.first_name} ${found.last_name}` })
          if (!found.checkin_at) markEntry(found.id)
        } else {
          setScanResult({ ok: false, name: '' })
        }
        ...
        {eventId && (
          <>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
              ...
              <button
                onClick={() => { setEventId(''); setAttendees([]); setQuery('') }}
            ...
            {scanResult && (
              <div style={{
                marginBottom: '16px', padding: '16px 20px', borderRadius: '8px',
                background: scanResult.ok ? 'rgba(72,187,120,0.1)' : 'rgba(252,129,74,0.1)',
                border: `0.5px solid rgba(${scanResult.ok ? '72,187,120' : '252,129,74'},0.3)`,
                ...S, fontSize: '14px',
                color: scanResult.ok ? 'rgba(72,187,120,0.9)' : 'rgba(252,129,74,0.9)',
              }}>
                {scanResult.ok ? `✓ Acceso permitido — ${scanResult.name}` : '✗ QR no encontrado en este evento'}
              </div>
            )}
                ...
                {filtered.map((a) => {
                  ...
                  return (
                    <div key={a.id} style={{
                      ...
                      {/* Button */}
                      <button
                        onClick={() => checked ? undoEntry(a.id) : markEntry(a.id)}
                        disabled={isLoading}
                        style={{
